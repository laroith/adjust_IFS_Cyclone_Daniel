...
Example use:
python apply_CC_cas.py --cas_input_dir=/path/to/cas_files --sst_file_path=/path/to/sst_file.nc --output_dir=/path/to/output_dir
"""
...
def process_file(cas_file_path, sst_file_path, output_dir):
    ...
    sst = sst_clim['mean_sst'].fillna(273.15) 
    #sst = sst.isel(time=0)
   ...
   # interpolated_sst_da = xr.DataArray(np.expand_dims(interpolated_sst, axis=-1), 
    ...
    ts_mixed = (sst * (1 - fr_land) + ts * fr_land)
    
    ocean_condition = np.isfinite(sst) & (fr_land <= 0.05)
    land_condition = np.isfinite(ts) & (fr_land >= 0.95)
    mixed_condition = np.isfinite(ts) & np.isfinite(ts_mixed) & (fr_land > 0.05) & (fr_land < 0.95) & (sst > 273.15)
    ...
    file_cas_new['T_SKIN'] = xr.where(ocean_condition, sst, file_cas_new['T_SKIN'], keep_attrs=True)
    file_cas_new['T_SKIN'] = xr.where(land_condition, ts, file_cas_new['T_SKIN'], keep_attrs=True)
