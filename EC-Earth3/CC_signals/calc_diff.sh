...
for scen_file in *scen.nc; do
    # Identify the variable name by cutting the string up to '_EC3_scen.nc'
    var_name=$(echo "$scen_file" | sed 's/_EC3_scen.nc//')

    # Construct the historical file name
    hist_file="${var_name}_EC3_hist.nc"

    # Construct the output difference file name
    diff_file="${var_name}_EC3_diff.nc"
