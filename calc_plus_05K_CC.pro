PRO main
  ...
  FILE_COPY, 'CC_'+VAR[0]+'_event.nc' , 'CC_'+VAR[0]+'_event.nc_plus_05K', /OVERWRITE
  FILE_COPY, 'CC_'+VAR[0]+'_SPIP.nc' , 'CC_'+VAR[0]+'_SPIP.nc_plus_05K', /OVERWRITE
  FILE_COPY, 'mrso_CC_'+VAR[0]+'_October.nc' , 'mrso_CC_'+VAR[0]+'_October.nc_plus_05K', /OVERWRITE
  FILE_COPY, 'mrso_CC_'+VAR[0]+'_SPIP.nc' , 'mrso_CC_'+VAR[0]+'_SPIP.nc_plus_05K', /OVERWRITE
  ...
  file=NCDF_OPEN('CC_'+VAR[0]+'_event.nc_plus_05K', /WRITE)
  ...
  ; open file, to be modified
  file=NCDF_OPEN('mrso_CC_'+VAR[0]+'_October.nc_plus_05K', /WRITE)
  ID=NCDF_VARID(file,'mrso')
  NCDF_VARGET,file, ID, mrso   
  mrso[*,*,*] = mrso[*,*,*] ^ (- tas_CC[0] * 0.5D)
  NCDF_VARPUT,file, ID, mrso

  NCDF_CLOSE,file
  
  ; open file, to be modified
  file=NCDF_OPEN('mrso_CC_'+VAR[0]+'_SPIP.nc_plus_05K', /WRITE)
  ID=NCDF_VARID(file,'mrso')
  NCDF_VARGET,file, ID, mrso   
  mrso[*,*,*] = mrso[*,*,*] ^ (- tas_CC[0] * 0.5D)
  NCDF_VARPUT,file, ID, mrso
